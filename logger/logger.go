...
// 设置日志文件，"/var/log/test/test.log"
func (l *Logger)file(path string) {
	if strings.HasPrefix(path, "/") != true { // 检查字符串开头
		fmt.Println("日志文件字符串必须以/开头")
	}
	if strings.HasSuffix(path, "/") {  // 检查字符串末尾
		fmt.Println("日志文件字符串必须不能用/结尾")
	}

	path_len := strings.LastIndexByte(path, '/')
	path_len++
	buf := []byte(path)
	pathDir := buf[:path_len]

	mkdirlog(string(pathDir))
	l.open(path)
}

func (l *Logger)open(file string) {
	var err error
	l.out, err = os.OpenFile(file, os.O_RDWR|os.O_CREATE|os.O_APPEND, os.ModePerm)
	if err != nil {
		return
	}
}
